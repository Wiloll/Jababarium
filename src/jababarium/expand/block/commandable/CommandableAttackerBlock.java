...
public abstract class CommandableAttackerBlock extends CommandableBlock {
    ...
    public void setStats() {
        super.setStats();
        // stats.add(Stat.instructions, t ->
        // t.add(Core.bundle.format("mod.ui.support-logic-control", "shootp", "\n 1 ->
        // Control All\n 2 -> Control Single")));
    ...
    public abstract class CommandableAttackerBlockBuild extends CommandableBlockBuild {
        ...
        public void control(LAccess type, Object p1, double p2, double p3, double p4) {
            // if(type == LAccess.shootp && timer.get(2, 10f) && (unit == null ||
            // !unit.isPlayer())){
            ...
            // velocity.scl((delayTime(tmpPoint.set(World.toTile(velocity.x),
            // World.toTile(velocity.y)).pack()) * Time.toSeconds + prepareDelay) /
            // 1.5f).add(target);
            // int pos = tmpPoint.set(World.toTile(velocity.x),
            // World.toTile(velocity.y)).pack();
            // if(Mathf.equal((float)p2,1))commandAll(pos);
            // if(Mathf.equal((float)p2,2) && canCommand(pos) &&
            // !isPreparing())command(pos);
            // }
            // }
